...
class ConvRFF(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        if not self.scale:
            if self.initializer == 'gaussian':
                ...
                raise ValueError(
                    f'Unsupported kernel initializer {self.initializer}')
    ...
    def _compute_mass(self,):
        ...
        mass_pos = tf.sqrt(trapz(tf.abs(mass_pos), ww_pos))
        return mass_pos
    ...
    def call(self, inputs):
        ...
        if self.normalization:
            outputs = tf.math.multiply(
                tf.math.sqrt(2/output_dim), tf.cos(outputs))
        else:
            outputs = tf.cos(outputs)

        outputs = tf.math.multiply(
            self._compute_mass(), outputs) if self.mass else outputs
        return outputs
